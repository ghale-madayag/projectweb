...
export function initSite() {
  ...
  function initWorkSticky() {
    ...
    // Desktop: detect which card is at the viewport center.
    const enableSync =
      hasGSAP && ScrollTrigger && !prefersReduced && window.innerWidth > 980;
    if (!enableSync) return; // mobile shows per-card captions via CSS

    cards.forEach((card, i) => {
      // Active sync (left text) — whichever card has reached the centre is active.

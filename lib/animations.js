...
export function initSite() {
  ...
  function initLenis() {
    ...
    lenis = new Lenis({
      ...
      smoothWheel: true,
      // Smooth out trackpad/wheel input across the whole site
      wheelMultiplier: 1,
      touchMultiplier: 1.5,
    });
    window.lenis = lenis;
    // IMPORTANT: drive Lenis from a SINGLE rAF source. With GSAP present we use
    // its ticker (and keep ScrollTrigger in sync); otherwise a plain rAF loop.
    // Running both at once double-steps Lenis and makes scrolling jittery.
  ...
  function initProcessTimeline() {
    ...
    // Desktop fans the cards into a staggered pile; on mobile/tablet the stagger
    // is removed so every card (incl. the last) rests full-width and readable.
    const offset = window.innerWidth <= 1024 ? 0 : 48;
  ...
  /* =======================================================
     ...
     ======================================================= */
  /* =======================================================
     SERVICES — click a service to pop up its image (GSAP)
     ======================================================= */
  function initServicesPopup() {
    const popup = document.getElementById("svcPopup");
    const imgEl = document.getElementById("svcPopupImg");
    if (!popup || !imgEl) return;
    const backdrop = popup.querySelector(".svc-popup__backdrop");
    const panel = popup.querySelector(".svc-popup__panel");
    ...
    function close() {
      ...
      gsap
        ...
        .to(panel, { opacity: 0, scale: 0.92, duration: 0.3, ease: "power2.in" }, 0)
        .to(backdrop, { opacity: 0, duration: 0.3, ease: "power2.in" }, 0.05);
    }

    services.forEach((svc) => {
      const img = svc.getAttribute("data-img");
      if (!img) return;
      const trigger = () => {
        const title = svc.querySelector(".service__title");
        open(`/Images/${img}.webp`, title ? title.textContent : "");
      };
      svc.addEventListener("click", trigger);
      svc.setAttribute("role", "button");
      svc.setAttribute("tabindex", "0");
    ...
    popup
      .querySelectorAll("[data-svc-close]")
  ...
  /* =======================================================
     SERVICES — hover a service to reveal its image as the
     section background, zooming in (pointer devices only).
     ======================================================= */
  function initServicesMedia() {
    if (isTouch) return; // touch keeps the click-to-popup behaviour
    const section = document.querySelector(".services");
    const media = document.getElementById("servicesMedia");
    if (!section || !media) return;
    const services = Array.from(document.querySelectorAll("[data-service]"));
    if (!services.length) return;

    // Circular mask values — 75% always covers a rectangle's corners.
    const REVEAL = "circle(75% at 50% 50%)";
    const HIDDEN = "circle(0% at 50% 50%)";

    // Build (and preload) one background layer per service.
    const layers = services.map((svc) => {
      const img = svc.getAttribute("data-img");
      const layer = document.createElement("div");
      layer.className = "services__media-layer";
      if (img) layer.style.backgroundImage = `url("/Images/${img}.webp")`;
    ...
    function show(i) {
      ...
      // Bounded z-index (0/1/2) so the scrim (z-index:5) always stays on top:
      // newest layer above the previous, previous above the rest.
      layers.forEach((l) => (l.style.zIndex = "0"));
      if (prev >= 0) layers[prev].style.zIndex = "1";
      layer.style.zIndex = "2";

      if (hasGSAP && !prefersReduced) {
        // Reset any stale layers (rapid hopping) but keep prev beneath as a base.
        layers.forEach((l, idx) => {
          if (idx !== i && idx !== prev) {
            gsap.killTweensOf(l);
            l.style.clipPath = HIDDEN;
          }
        });
        ...
        gsap.fromTo(
          ...
          { clipPath: HIDDEN },
          {
            ...
            ease: "power2.out",
            onComplete: () => {
              if (prev >= 0 && prev !== i) layers[prev].style.clipPath = HIDDEN;
            },
          }
        );
      } else {
        layers.forEach((l, idx) => (l.style.clipPath = idx === i ? REVEAL : HIDDEN));
      }
    }
    ...
    function hideAll() {
      ...
      if (hasGSAP && !prefersReduced) {
        gsap.killTweensOf(layer);
        gsap.to(layer, { clipPath: HIDDEN, duration: 0.5, ease: "power2.inOut" });
      } else {
        layers.forEach((l) => (l.style.clipPath = HIDDEN));
      }
    }

    services.forEach((svc, i) =>
      svc.addEventListener("mouseenter", () => show(i))
    );
    const list = document.querySelector(".services__list");
    if (list) list.addEventListener("mouseleave", hideAll);
  }

  /* =======================================================
     WORK — floating "skip to next section" button. Shown once
     the visitor passes the 5th project, and only while the Work
     section is still in view. Works on all screen sizes.
     ======================================================= */
  function initWorkSkip() {
    const skipEl = document.getElementById("workSkip");
    const section = document.getElementById("work");
    if (!skipEl || !section) return;
    const right = document.getElementById("workRight");
    const cards = right ? Array.from(right.querySelectorAll(".wk")) : [];
    if (cards.length < 5) return; // nothing to skip past
    if (!hasGSAP || !ScrollTrigger) return; // (click still works via anchors)

    const labelEl = skipEl.querySelector("span");
    let past5 = false;
    let inWork = false;
    let dir = 1; // 1 = scrolling down, -1 = scrolling up
    const update = () => skipEl.classList.toggle("is-shown", past5 && inWork);

    // Direction-aware: skip forward to Process when going down, or back up to
    // the previous section (Services) when the visitor is scrolling up.
    const applyDir = () => {
      if (dir === -1) {
        skipEl.setAttribute("href", "#services");
        if (labelEl) labelEl.textContent = "Skip back up";
      } else {
        skipEl.setAttribute("href", "#process");
        if (labelEl) labelEl.textContent = "Seen enough? Skip ahead";
      }
    };

    // Crossed the 5th project?
    ScrollTrigger.create({
      trigger: cards[4],
      start: "top center",
      onEnter: () => {
    ...
    // Still inside the Work section? (hide once it leaves the viewport)
    ScrollTrigger.create({
      trigger: section,
      start: "top top",
      end: "bottom bottom",
      onToggle: (self) => {
        inWork = self.isActive;
        update();
      },
    });
    // Track scroll direction to flip the button's label + target.
    ScrollTrigger.create({
      onUpdate: (self) => {
    ...
    // Smooth glide to the target. The skip distance is large, so use a longer
    // duration + ease-in-out instead of the default anchor scroll.
    const easeInOut = (x) =>
      x < 0.5 ? 4 * x * x * x : 1 - Math.pow(-2 * x + 2, 3) / 2;
    skipEl.addEventListener("click", (e) => {
      const id = skipEl.getAttribute("href");
      const target = id && document.querySelector(id);
      if (!target) return;
      e.preventDefault();
      if (lenis) lenis.scrollTo(target, { offset: 0, duration: 1.9, easing: easeInOut });
      else target.scrollIntoView({ behavior: "smooth" });
    });
  }

  /* =======================================================
     PROCESS CARDS — purple spotlight that follows the cursor
     (mirrors the hero shader's cursor glow). CSS draws the
     radial gradient from the --mx/--my we set here.
     ======================================================= */
